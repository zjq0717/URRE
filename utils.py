...
class ReplayBuffer(object):
    ...
    # def convert_D4RL(self, dataset, dataset_random, load_reward=False, mix_ratio=0.8):
    #     # Number of samples to take from each dataset
    #     size_dataset = dataset['observations'].shape[0]
    #     size_random = dataset_random['observations'].shape[0]
    #
    #     # Calculate how many samples to take from each dataset based on the mix ratio
    #     size_mix_dataset = int(size_dataset * mix_ratio)
    #     size_mix_random = size_dataset - size_mix_dataset
    #     print(size_mix_dataset)
    #     print(size_mix_random)
    #     # Sample the datasets based on the mix ratio
    #     state_dataset = dataset['observations'][:size_mix_dataset]
    #     action_dataset = dataset['actions'][:size_mix_dataset]
    #     next_state_dataset = dataset['next_observations'][:size_mix_dataset]
    #     reward_dataset = dataset['rewards'][:size_mix_dataset]
    #     terminal_dataset = dataset['terminals'][:size_mix_dataset]
    #
    #     state_random = dataset_random['observations'][:size_mix_random]
    #     action_random = dataset_random['actions'][:size_mix_random]
    #     next_state_random = dataset_random['next_observations'][:size_mix_random]
    #     reward_random = dataset_random['rewards'][:size_mix_random]
    #     terminal_random = dataset_random['terminals'][:size_mix_random]
    #
    #     # Concatenate the data from both datasets
    #     self.state = np.concatenate([state_dataset, state_random], axis=0)
    #     self.action = np.concatenate([action_dataset, action_random], axis=0)
    #     self.next_state = np.concatenate([next_state_dataset, next_state_random], axis=0)
    #
    #     if load_reward:
    #         self.reward = np.concatenate([reward_dataset, reward_random], axis=0).reshape(-1, 1)
    #         self.raw_reward = self.reward
    #     else:
    #         self.raw_reward = np.concatenate([reward_dataset, reward_random], axis=0).reshape(-1, 1)
    #
    #     self.ctrl_reward = np.sum(np.square(self.action), axis=1)
    #
    #     self.not_done = 1. - np.concatenate([terminal_dataset, terminal_random], axis=0).reshape(-1, 1)
    #     self.size = self.state.shape[0]

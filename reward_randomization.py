...
def reward_randomization_nn(replay_buffer, state_dim, action_dim, reward_dim=100, batch_size=256, scale=1,
    ...
    #
    model_state_dict = torch.load('./reward/random_reward_net_hop.pth')
    for i, net in enumerate(random_reward_net):

        net.load_state_dict(model_state_dict[f'net_{i}'])
    print("load-reward")
    # model_state_dict = {}
    # for i, net in enumerate(random_reward_net):
    #     model_state_dict[f'net_{i}'] = net.state_dict()
    # print("save")
    # # 保存所有网络的状态字典
    # torch.save(model_state_dict, './reward/random_reward_net_hop.pth')
    # if load_model is not None:
    #     for net in random_reward_net:
    #         net.load(f"./models/random_reward_net_walk.pth")
    # random_reward_net = [RandomReward(state_dim, action_dim) for _ in range(max(256, reward_dim))]
    random_rewards = compute_random_reward_nn(replay_buffer, random_reward_net, batch_size)

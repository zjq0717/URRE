...
def sample_batch(dataset, random_rewards,  batch_size):
    ...
    batch = {k: v[indices].cuda() for k, v in dataset.items()}
    randoms = random_rewards[:, indices].cuda()
...
def eval_policy(env, env_name, alg, max_episode_steps, n_eval_episodes):
    ...
    return_mean = round(eval_returns.mean(), 1)
    return_std = round(eval_returns.std(), 1)
    normalized_return_mean = round(normalized_returns.mean(), 1)
    normalized_return_std = round(normalized_returns.std(), 1)
    return return_mean, return_std, normalized_return_mean, normalized_return_std

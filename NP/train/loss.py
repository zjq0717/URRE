...
def compute_elbo(Y_D, p_Y, q_D_G, q_C_G, config, logger=None):


    # result_dict = {}
    # for i in range(Y_D.size(1)):
    #     result_dict[i] = Y_D[0, i, :, :].unsqueeze(0)  # 提取 X 的每一部分，并将其添加到字典中

    '''
    Compute (prior-approximated) elbo objective for NP-based models.
    '''

    log_prob = Normal(p_Y[0], p_Y[1]).log_prob(Y_D).mean(0).sum()
        # else:
    ...
    elbo = log_prob - (config.beta_G*kld_G)
    return elbo
